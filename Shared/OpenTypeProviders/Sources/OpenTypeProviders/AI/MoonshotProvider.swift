     1|import Foundation
     2|
     3|public actor MoonshotProvider: AIProvider {
     4|    public let name = "Moonshot"
     5|    private let baseURL = "https://api.moonshot.cn/v1"
     6|
     7|    public init() {}
     8|
    ...
    31|        }
    32|
    33|        let result = try JSONDecoder().decode(MoonshotResponse.self, from: data)
    ...
    59|        }
    60|
    61|        let result = try JSONDecoder().decode(MoonshotResponse.self, from: data)
    62|        return result.choices.first?.message.content ?? text
    63|    }
    64|}
    65|
    66|private struct MoonshotResponse: Codable {
