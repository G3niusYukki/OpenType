...
vi.mock('electron', () => {
  ...
  const mockApp = { getPath: vi.fn().mockReturnValue('/tmp'), whenReady: vi.fn().mockResolvedValue(undefined), on: vi.fn(), once: vi.fn(), quit: vi.fn(), isReady: vi.fn().mockReturnValue(true), getVersion: vi.fn().mockReturnValue('0.3.0'), getName: vi.fn().mockReturnValue('OpenType') };
